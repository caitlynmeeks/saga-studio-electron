...
function candidates () {
  ...
  // 1b. the same runtime, built but not yet packaged — what `npm run
  // runtime` makes. Under npm start there is no bundle, and without this
  // a fresh checkout falls through to a system python that has none of
  // the studio's packages and no kokoro-onnx.
  if (!app.isPackaged) {
    out.push(path.join(app.getAppPath(), 'build', 'runtime', 'python', 'bin', 'python3'))
  }
...
// ── the updatable payload ───────────────────────────────────────────────
// studio.py can fetch its own newer self from GitHub. It writes that copy
// HERE rather than into the .app, for two reasons: a bundle is read-only in
// spirit (and on some installs in fact), and writing into a signed one
// would break its signature. This folder therefore outranks the bundled
// copy whenever it holds something newer.
/** The folder a downloaded update lands in. Always passed to studio.py. */
function appPayload () {
  return path.join(app.getPath('userData'), 'app')
}

/** When the payload was cut, as a unix time — 0 when there is none. */
function payloadStamp (dir) {
  try {
    const v = JSON.parse(fs.readFileSync(path.join(dir, 'version.json'), 'utf8'))
    return Number(v.at) || 0
  } catch (_) { return 0 }
}

/** Which studio.py to run: the downloaded payload when it is complete AND
    newer than the bundled one, else the bundle. A fresh install carrying a
    later build therefore wins over a stale download, which is the case that
    would otherwise strand someone on old code after reinstalling. */
function studioScript (bundledDir) {
  const dir = appPayload()
  const downloaded = path.join(dir, 'studio.py')
  if (!isFile(downloaded)) return path.join(bundledDir, 'studio.py')
  if (payloadStamp(dir) <= payloadStamp(bundledDir)) {
    return path.join(bundledDir, 'studio.py')
  }
  return downloaded
}

/** The bundled uv, or null — the engine manager falls back to pip. In an
    unpackaged run the same uv sits in build/runtime, where `npm run
    runtime` put it. */
function uv () {
  const spots = []
  if (process.resourcesPath) spots.push(path.join(process.resourcesPath, 'uv', 'uv'))
  if (!app.isPackaged) spots.push(path.join(app.getAppPath(), 'build', 'runtime', 'uv', 'uv'))
  for (const p of spots) if (isFile(p)) return p
  return null
}

/** The bundled Kokoro model dir, or null when the library has its own copy —
    which wins, since it may hold a bigger-precision model. Unpackaged, the
    "bundle" is build/models — where `npm run runtime` puts the files;
    process.resourcesPath exists under npm start too, but points into
    Electron's own bundle, which never has models. */
function kokoroDir (libraryDir) {
  ...
  const spots = []
  if (process.resourcesPath) spots.push(path.join(process.resourcesPath, 'models', 'kokoro'))
  if (!app.isPackaged) spots.push(path.join(app.getAppPath(), 'build', 'models', 'kokoro'))
  for (const b of spots) if (isFile(path.join(b, 'voices-v1.0.bin'))) return b
  return null
}
...
module.exports = {
  ...
  engines, uv, kokoroDir, appPayload, studioScript, payloadStamp
}
