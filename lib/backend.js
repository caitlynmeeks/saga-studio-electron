...
// The app's own home port. The page keeps its per-machine taste — which
// shelves are folded, the active tab, the sidebar's width — in localStorage,
// and localStorage is filed under the origin, port number included. A fresh
// port every launch was a fresh origin every launch: the studio came up with
// every shelf flung open and every setting back at its default, having
// faithfully saved them all somewhere it would never look again.
const HOME_PORT = 5012
...
/** True if nothing on this machine is holding port `p` right now. */
function portFree (p) {
...
/**
 * The same port as every other launch, so the origin — and everything the
 * page remembers under it — holds still. Preference order: whatever port
 * this app used last time (config), then the home port, then any free one
 * as a last resort. The single-instance lock means the usual thief of our
 * own saved port (a second copy of the app) cannot exist, so in practice
 * the first answer is the answer.
 */
async function stablePort () {
  const saved = config.get('port', 0)
  for (const p of [saved, HOME_PORT]) {
    if (!Number.isInteger(p) || p <= 1024 || p === CLASSIC_PORT) continue
    if (await portFree(p)) {
      if (p !== saved) config.save({ port: p })
      return p
    }
  }
...
async function start ({ onExit } = {}) {
  ...
  // The bundled python, unless a downloaded update sitting in Application
  // Support is newer — see paths.studioScript. The .app is never written to.
  const bundled = path.join(app.getAppPath(), 'python')
  const script = paths.studioScript(bundled)
  if (!paths.isFile(script)) throw new Error(`studio.py is missing at ${script}`)

  state.port = await stablePort()
  ...
  const env = Object.assign({}, process.env, {
    ...
    // per-machine venvs for the downloadable engines — the engine manager
    // in studio.py builds and reads them here
    SAGA_ENGINES: paths.engines(),
    // where studio.py writes a newer self when the author installs an
    // update: beside the engines, never inside the .app
    SAGA_APP_DIR: paths.appPayload(),
    // package.json's version, for the header chip — the payload commit is
    // the studio's own business, this is the shell's half of the answer
    SAGA_APP_VERSION: app.getVersion(),
  ...
  const uv = paths.uv()
  if (uv) env.SAGA_UV = uv
  const kokoro = paths.kokoroDir(library)
  if (kokoro) env.SAGA_KOKORO_DIR = kokoro

  note(`python: ${py.path}${py.source === 'bundled' ? '  (bundled runtime)'
    : py.torch ? '' : '  (no torch found beside it)'}`)
  ...
  try {
    ...
    // the bundled runtime is torchless by design, so its absence explains
    // nothing there — only a hand-picked or fallback interpreter earns the
    // "that Python cannot run the voice model" page
    if (!py.torch && py.source !== 'bundled') {
      err.kind = 'no-torch'
